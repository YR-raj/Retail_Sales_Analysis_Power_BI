...
The dataset used for this analysis is the **Online Retail II dataset** from the UCI Machine Learning Repository, which contains transactional data for a UK-based online retail store over two years.
...
* A noticeable decline in **December** is observed, which may be due to **post-peak normalization or partial data availability**.
...
* A small group of products contributes significantly to overall revenue, indicating a **skewed product distribution**.
* Certain products show **extremely high return rates (>90%)**, suggesting:
...
* Customer analysis includes both **registered and guest users**, with missing customer IDs treated as "Guest" to preserve data integrity.
* Key KPIs analyzed:
  ...
  * Average Order Value (AOV)
  * Revenue per Customer
* The presence of repeat purchasing behavior indicates **customer retention opportunities**.
...
* Leverage **seasonal trends (especially November)** for targeted marketing campaigns.
...
* A small subset of products drives the majority of revenue, aligning with the **Pareto principle (80/20 rule)**.
